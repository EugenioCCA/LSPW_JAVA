...
public class LoginControlador extends HttpServlet {
		...
		if(cookies!= null) {
			for(int i = 0; i<cookies.length; i++) {
				...
				if(cookie.getValue().equals("username")) {
					System.out.println("Una cookie"+cookie.getValue());
					email = cookie.getValue(); 
				}
						
			}
		}
		response.sendRedirect(request.getContentType()+"/vista/login.jsp?email="+email);
	}
	...
	private void ingresar(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if (user.isValidoEmail()) {
			Cookie cookie = new Cookie("email",username); 
			cookie.setMaxAge(120); //Tiempo de vida de la cookie en el navegador
			
			response.addCookie(cookie);
			response.sendRedirect(request.getContextPath() + "/vista/privado/panel.jsp?email="+username+"&opcionPanel=panel");
